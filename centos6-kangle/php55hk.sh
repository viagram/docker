...
wget --no-check-certificate -c http://hk1.php.net/get/php-5.5.28.tar.gz/from/this/mirror -O php-5.5.28.tar.gz
...
if [ ! -f $PREFIX/config.xml ]; then
        wget --no-check-certificate -c $DOWNLOAD_PHP_URL/php55/config.xml -O $PREFIX/config.xml
fi
cd ..
wget --no-check-certificate -c $DOWNLOAD_PHP_URL/php55/php-templete.ini -O $PREFIX/php-templete.ini
#install ioncube
wget --no-check-certificate -c https://raw.githubusercontent.com/1265578519/kangle/master/php/5.5/5527/ioncube-$ZEND_ARCH-5.5.zip
