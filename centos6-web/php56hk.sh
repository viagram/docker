...
wget --no-check-certificate -c http://hk2.php.net/get/php-5.6.12.tar.bz2/from/this/mirror  -O php-5.6.12.tar.bz2
...
if [ ! -f $PREFIX/config.xml ]; then
        wget --no-check-certificate -c $DOWNLOAD_PHP_URL/php56/config.xml -O $PREFIX/config.xml
fi
cd ..
wget --no-check-certificate -c $DOWNLOAD_PHP_URL/php56/php-templete.ini -O $PREFIX/php-templete.ini
#install ioncube
wget --no-check-certificate -c https://raw.githubusercontent.com/1265578519/kangle/master/php/5.6/5611/ioncube-$ZEND_ARCH-5.6.zip
...
wget --no-check-certificate -c http://pecl.php.net/get/memcache-3.0.8.tgz
...
make install
cd ..
wget --no-check-certificate -c http://pecl.php.net/get/apcu-4.0.7.tgz
